...
# For Euclidian distances
if (metric=="Euclidean") mat_funct <-scale(mat_funct) # scaling if continuous traits

# For Gower distances
for (i in 1:ncol(mat_funct)) if (is.character(mat_funct[,i])) mat_funct[,i]<-as.factor(mat_funct[,i]) #tranform character variable as factor 
if (metric == "Gower") mat_dissim<-daisy(mat_funct, metric=tolower(metric), weights=traits_weights )
